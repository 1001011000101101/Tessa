...
Скорость (соответственно, стоимость) внедрения системы TESSA будет страдать от без-ORM-ного подхода. Неочевидно, но факт!
Справедливости ради нужно заметить, что в коде замечен нэймспейс LinqToDB, но как отдельной библиотеки не существует, и какая там версия, остается только гадать.
